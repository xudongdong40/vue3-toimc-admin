  ...
  export default defineComponent({
    ...
    setup(props, { emit, attrs, expose }) {
      ...
      return () => {
        ...
        return (
          <div class="img-rotate-verify">
            <div class={'img-rotate-verify-img__wrap'} style={unref(getImgWrapStyleRef)}>
              <img
                src={src}
              ...
              />
              {state.showTip && (
                <span class={['img-rotate-verify-img__tip', state.isPassing ? 'success' : 'error']}>
                  {state.isPassing
                    ? t('components.verify.time', { time: time.toFixed(1) })
                    : t('components.verify.error')}
                </span>
              )}
              {!state.showTip && !state.draged && (
                <span class={['img-rotate-verify-img__tip', 'normal']}>
                  {t('components.verify.redoTip')}
                </span>
              )}
            </div>
            <BasicDragVerify
              class={'img-rotate-verify-drag__bar'}
...
<style lang="scss">
  .img-rotate-verify {
    ...
    &-img__tip {
      ...
      &.success {
        background-color: rgba(#529b2e, 60%);
      }

      &.error {
        background-color: rgba(#f56c6c, 60%);
      }
