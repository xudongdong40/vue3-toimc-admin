<template>
  <div class="p-1">
    <el-select v-model="province" placeholder="请选择省份" class="mr-1" :clearable="clearable">
      <el-option v-for="item in lists" :key="item.code" :value="item.code" :label="item.name"></el-option>
    </el-select>
    <el-select
      v-model="city"
      :disabled="!province"
    ...
    <el-select v-model="area" :disabled="!city" placeholder="请选择区域" :clearable="clearable">
      <el-option v-for="item in areaList" :key="item.code" :value="item.code" :label="item.name"></el-option>
    </el-select>
  </div>
</template>
...
export default defineComponent({
  ...
  props: {
    ...
    url: {
      type: String,
      default: 'https://github.91chi.fun/https://raw.githubusercontent.com/modood/Administrative-divisions-of-China/master/dist/pca.json'
    },
    online: {
      type: Boolean,
      default: false
  ...
  setup(props, { emit }) {
    ...
    onBeforeMount(() => {
      online.value && fetch(url.value).then(res => res.json()).then(out => lists.value = out)
    ...
    watch(area, () => {
      if (area.value) {
        ...
        emit('change', plain.value ? `${provinceName} ${cityName} ${areaName}` : {
          province: {
            code: province.value,
            name: provinceName
          },
