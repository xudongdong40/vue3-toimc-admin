  ...
  <el-drawer v-model="show" :direction="direction" @close="onClose">
    ...
    <template #default>
      <div class="border-bg-gray-200 p-4 border overflow-auto">
        <el-form ref="vForm" :model="formData" :rules="rules" label-position="right" label-width="120px" size="default"
          @submit.prevent>
          <el-form-item label="菜单类型：" prop="menuType">
            <el-radio-group v-model="formData.menuType">
              <el-radio v-for="(item, index) in menuTypeOptions" :key="index" :label="item.value" :style="{
                display: 'inline'
              }">{{ item.label }}</el-radio>
            </el-radio-group>
          </el-form-item>
          ...
          <template v-if="formData.menuType == 2">
            ...
            <el-form-item label="授权策略：" prop="permsType">
              <el-radio-group v-model="formData.permsType">
                <el-radio v-for="(item, index) in permsTypeOptions" :key="index" :label="item.value" :style="{
                  display: 'inline'
                }">{{ item.label }}</el-radio>
              </el-radio-group>
            </el-form-item>
            <el-form-item label="状态：" prop="status">
              <el-radio-group v-model="formData.status">
                <el-radio v-for="(item, index) in statusOptions" :key="index" :label="item.value" :style="{
                  display: 'inline'
                }">{{ item.label }}</el-radio>
              </el-radio-group>
            </el-form-item>
          ...
          <template v-else>
            ...
            <template v-else-if="formData.menuType == 1">
              <el-form-item label="上级菜单：" prop="parentId" class="required">
                <!-- <el-input v-model="formData.parentId" type="text" clearable></el-input> -->
                <!-- 树控件有问题 2022.4.7 -->
                <el-tree-select v-model="formData.parentId" :render-after-expand="false"
              ...
              <el-input v-model="formData.icon" type="text" placeholder="点击选择图标" clearable>
                <template #append>
                  <el-popover placement="bottom" :width="400" trigger="click">
                    ...
                    <el-input v-model="searchIconKey" class="my-2" type="text" placeholder="点击搜索图标" clearable>
                    </el-input>

                    <div class="icon-container overflow-auto">
                      <icon-list classes="p-2 w-1/8" :items="menuIconList" :show-text="false" :choose="formData.icon"
                        @click="onPickIcon"></icon-list>
                    </div>
                  </el-popover>
            ...
            <el-form-item label="打开方式" prop="internalOrExternal">
              <el-switch v-model="formData.internalOrExternal" :width="50" inline-prompt active-text="外部"
...
export default defineComponent({
  ...
  setup(props, ctx) {
    ...
    return {
      show, treeProps, onPickIcon, menuIconList, searchIconKey,
      // showDrawer,isUpdate,
