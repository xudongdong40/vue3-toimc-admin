<template>
  <el-dialog :model-value="dialogFormVisible" title="新增" width="520px" center @close="close" @opened="opened">
    <el-form ref="ruleFormRef" :model="formData" :rules="rules">
      ...
      <el-form-item v-if="parentId != ''" label="上级部门" :label-width="formLabelWidth">
        <el-tree-select v-model="formData.parentId" clearable node-key="id" :data="treeData" default-expand-all
      ...
      <el-form-item label="排序" :label-width="formLabelWidth">
        <el-input-number v-model="formData.departOrder" :min="0" :max="9999999999" controls-position="right" />
      </el-form-item>
...
<script lang="ts" >
// watch, computed, in  ct, re  f, unref, onMounted
...
export default defineComponent({
  ...
  setup(_props, { emit }) {
    ...
    const opened = () => {
      formData.departName = ''
      formData.parentId = '10000'//_props.parentId
    ...
    const saveDepartInfo = () => {
      loading.value = true
      saveOrUpdateDepart(formData).then((res) => {
        loading.value = false
        if (res.code == 200) {
          ElMessage({
            message: `${res.message}`,
    ...
    return {
      ruleFormRef, rules, defaultProps,
      formData,
      formLabelWidth, loading,
      opened,
      close, submitForm
    }
