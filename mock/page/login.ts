...
export default [
  // mock user login
  {
    ...
    response: ({ body }) => {
      ...
      const checkUser = createFakeUserList.filter(
        (item) => [item.username, item.phone].includes(username) && password === item.password
      )
      if (!checkUser) {
        return resultError('Incorrect account or password！')
      }
      const { userId, username: _username, token, realname, desc, roles, avatar } = checkUser[0]
  ...
  },
  {
    ...
    response: ({ body }) => {
      ...
      const { userId, username, token, realname, desc, roles, avatar } = createFakeUserList[0]
      return resultSuccess({ userId, username, token, realname, desc, roles, avatar })
    }
  },
  {
